defmodule MtgTreacheryWeb.CoreComponents do
  @moduledoc """
  ...
  import MtgTreacheryWeb.Gettext
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="bg-slate-800 shadow-zinc-700/10 ring-zinc-700/10 relative hidden rounded-2xl p-14 shadow-lg ring-1 transition bg-slate-800"
            >
              <div class="absolute top-6 right-5">
                ...
                >
                  <.icon name="hero-x-mark-solid bg-zinc-200" class="h-5 w-5" />
                </button>
              </div>
              <div id={"#{@id}-content"}>
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 rounded-lg bg-indigo-700 hover:bg-indigo-900 py-2 px-3",
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-1 block w-full rounded-md border border-gray-300 bg-white shadow-sm focus:border-zinc-400 focus:ring-0 sm:text-sm text-zinc-800"
        multiple={@multiple}
        {@rest}
      >
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(MtgTreacheryWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(MtgTreacheryWeb.Gettext, "errors", msg, opts)
    end
  end
