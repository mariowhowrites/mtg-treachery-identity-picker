defmodule MtgTreacheryWeb.IdentityLive.IdentityCard do
  ...
  def show(assigns) when assigns.selected_player.identity_name != nil do
    ~H"""
    <div
      id="identity-card"
    ...
    >
      ...
      >
        <div id="identity-card-content" class="h-full flex flex-col items-center justify-between">
          ...
          >
            <h3 class="text-lg font-bold text-zinc-800"><%= @selected_player.identity_name %></h3>
          </section>
          <div class="flex flex-col items-center bg-stone-100 text-zinc-800 mx-4 mb-4">
            <div class="border-b-2 border-stone-200 w-full text-center py-1 font-semibold">
              <%= @selected_player.identity_role %>
            </div>
            <div class="px-2 rounded-sm mb-4 text-sm pt-2">
              <%= if @selected_player.identity_role != "Leader" do %>
                <div>Unveil cost: <%= @selected_player.identity_unveil_cost %></div>
              <% end %>
              <%= for paragraph <- String.split(@selected_player.identity_description, "\n") do %>
  ...
  defp identity_card_front_classes(selected_player) do
    [
      ...
      role_background(selected_player.identity_role)
    ]
  end
